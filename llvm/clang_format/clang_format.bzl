...
```python
load("@cc_toolchains//llvm/clang_format:clang_format.bzl", "clang_format_toolchain")
...
toolchain(
    ...
    toolchain_type = "@cc_toolchains//llvm/clang_format:toolchain_type",
)
```
...
```text
build --aspects=@cc_toolchains//llvm/clang_format:clang_format.bzl%clang_format_aspect
...
The `clang_format_aspect` also uses a `--@cc_toolchains//llvm/clang_format:clang_format_config` setting which determines
the [configuration file][so] used by the formatter (`@cc_toolchains//llvm/clang_format`) and the aspect
...
```text
build --@cc_toolchains//llvm/clang_format:clang_format_config=//:.clang-format
```
...
load("//llvm/clang_format/private:clang_format_utils.bzl", "BuildSettingInfo")
...
clang_format_toolchain = rule(
    ...
    attrs = {
        ...
        "_is_windows": attr.label(
            ...
            cfg = "exec",
            default = Label("//llvm/clang_format/private:is_windows"),
        ),
    },
)

def _current_clang_format_toolchain_impl(ctx):
    toolchain = ctx.toolchains[str(Label("//llvm/clang_format:toolchain_type"))]
...
current_clang_format_toolchain = rule(
    ...
    toolchains = [
        str(Label("//llvm/clang_format:toolchain_type")),
    ],
...
def _perform_check(ctx, target, srcs):
    ...
    """
    toolchain = ctx.toolchains[Label("//llvm/clang_format:toolchain_type")]
...
""",
    attrs = {
        "_config": attr.label(
            ...
            default = Label("//llvm/clang_format:clang_format_config"),
        ),
        "_extensions": attr.label(
            ...
            default = Label("//llvm/clang_format:clang_format_extensions"),
        ),
        "_process_wrapper": attr.label(
            doc = "A process wrapper for running clang-format on all platforms",
            cfg = "exec",
            executable = True,
            default = Label("//llvm/clang_format/private:clang_format_process_wrapper"),
        ),
        "_source_attrs": attr.label(
            ...
            default = Label("//llvm/clang_format:clang_format_source_attrs"),
        ),
    },
    incompatible_use_toolchain_transition = True,
    fragments = ["cpp"],
...
def clang_format(
        name,
        config = Label("//llvm/clang_format:clang_format_config"),
        types = "^cc_",
        **kwargs):
    ...
    current_toolchain = Label("//llvm/clang_format:current_clang_format_toolchain")
    extensions_manifest = Label("//llvm/clang_format/private:clang_format_extensions_file")
    ...
    cc_binary(
        name = name,
        srcs = [
            Label("//llvm/clang_format/private:clang_format_runner.cc"),
        ],
        args = args,
        data = [
            config,
        ...
        deps = [
            ...
            Label("//llvm/clang_format/private:clang_format_utils"),
        ],
        **kwargs
    )
