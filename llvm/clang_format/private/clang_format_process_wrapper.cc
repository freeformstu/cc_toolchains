...
{
    const std::string working_dir = "__clang_format__";
    ...
    // Create copies of the source files to format
    ...
    const std::string pwd = get_working_path();
    const std::string clang_format_path = pwd + FILE_SEP + clang_format_file;
...
int main(int argc, char **argv)
{
    ...
    int exit_code = run_clang_format(
        exec_path, arguments, config_file, sources, diff_tool_file);
    if (exit_code)
    {
        return exit_code;
    }
    ...
    if (!touch_file.empty())
    {
        ...
        {
            std::cerr
                << "process wrapper error: failed to create touch file: \""
                << touch_file << "\"\n";
            return -1;
        }
        file.close();
